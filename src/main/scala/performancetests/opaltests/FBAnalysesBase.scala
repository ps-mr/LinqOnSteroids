    ...
    sourceMethod.body.get.instructions.exists {
        instr => {
             if_#(instr.isInstanceOf_#[INVOKEINTERFACE]) {
                letExp(instr.asInstanceOf_#[INVOKEINTERFACE])(invoke =>
                ...
                letExp(instr.asInstanceOf_#[INVOKEVIRTUAL])(invoke =>
                  invoke.declaringClass ==# targetClass.thisClass &&
                  invoke.name ==# targetMethod.name &&
                  invoke.methodDescriptor ==# targetMethod.descriptor)
             } else_# if_#(instr.isInstanceOf_#[INVOKESTATIC]) {
                letExp(instr.asInstanceOf_#[INVOKESTATIC])(invoke =>
                  invoke.declaringClass ==# targetClass.thisClass &&
                  invoke.name ==# targetMethod.name &&
                  invoke.methodDescriptor ==# targetMethod.descriptor)
             } else_# if_#(instr.isInstanceOf_#[INVOKESPECIAL]) {
                letExp(instr.asInstanceOf_#[INVOKESPECIAL])(invoke =>
                  invoke.declaringClass ==# targetClass.thisClass &&
                  invoke.name ==# targetMethod.name &&
                  invoke.methodDescriptor ==# targetMethod.descriptor)
             } else_# {
             ...
             /*
           (instr.isInstanceOf_#[INVOKEINTERFACE] && letExp(instr.asInstanceOf_#[INVOKEINTERFACE])(invoke => invoke.declaringClass ==# targetClass.thisClass && invoke.name ==# targetMethod.name && invoke.methodDescriptor ==# targetMethod.descriptor )) ||
           (instr.isInstanceOf_#[INVOKEVIRTUAL] && letExp(instr.asInstanceOf_#[INVOKEVIRTUAL])(invoke => invoke.declaringClass ==# targetClass.thisClass && invoke.name ==# targetMethod.name && invoke.methodDescriptor ==# targetMethod.descriptor )) ||
           (instr.isInstanceOf_#[INVOKESTATIC] && letExp(instr.asInstanceOf_#[INVOKESTATIC])(invoke => invoke.declaringClass ==# targetClass.thisClass && invoke.name ==# targetMethod.name && invoke.methodDescriptor ==# targetMethod.descriptor )) ||
           (instr.isInstanceOf_#[INVOKESPECIAL] && letExp(instr.asInstanceOf_#[INVOKESPECIAL])(invoke => invoke.declaringClass ==# targetClass.thisClass && invoke.name ==# targetMethod.name && invoke.methodDescriptor ==# targetMethod.descriptor ))
           */
        }
    }
  }
