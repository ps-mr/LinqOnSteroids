...
trait FBAbstractClassesThatDefinesCovariantEquals {
  ...
  private def analyzeSQuOptWithoutAbstractions() = {
    ...
    for {
      ...
      if parameterTypes.length ==# 1 && parameterTypes.headOption.fold(false) (_ ==# classFile.thisClass) // _ !=# ObjectType.Object
    } yield (classFile.thisClass, method.name, method.descriptor)
  }
  ...
  private def analyzeSQuOptWithAbstractions() = {
    ...
    for {
      ...
      if parameterTypes.length ==# 1 && parameterTypes.headOption.fold(false) (_ ==# methodRecord.classFile.thisClass) // _ !=# ObjectType.Object
