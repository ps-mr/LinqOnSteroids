...
trait RemoteFramework {
  ...
  implicit def autoMove[T: Serializator](value: Host#Rep[T])(implicit dest: Host): dest.Rep[T] = moveTo(value, dest)
...
//    v1 + v2
    //Wrappers for methods should incorporate the call to autoMove, probably. That way, though, they get longer (because
