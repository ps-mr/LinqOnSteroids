...
abstract class CommOp[T](x: Exp[T], y: Exp[T])(op: (T, T) => T) extends BinaryOpSymmExp[T, T](x, y) with CommutativeOp[T] {
...
case class Plus[T](override val t1: Exp[T], override val t2: Exp[T])(implicit val isNum: Numeric[T]) extends CommOp[T](t1, t2)(_ + _) {
  def copy(x: Exp[T], y: Exp[T]) = Plus(x, y)
}

case class Times[T](override val t1: Exp[T], override val t2: Exp[T])(implicit val isNum: Numeric[T]) extends CommOp[T](t1, t2)(_ * _) {
