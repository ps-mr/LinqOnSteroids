...
trait FoldPhysicalOperators {
  private def buildJoin[T, S, TKey, TResult](fmColl: Exp[Traversable[T]],
                                             ...
                                             moFun: FunSym[S, TResult], fmFun: FunSym[T, Traversable[TResult]],
                                             wfFun: FunSym[S, Boolean], origExp: Exp[Traversable[TResult]]): Exp[Traversable[TResult]] /*Join[T, S, TKey, TResult]*/ = {
  ...
  private def buildAntiJoin[T, S, TKey](filteredColl: Exp[Traversable[T]],
                                        ...
                                        lhs: Exp[TKey], rhs: Exp[TKey],
                                        filterFun: FunSym[T, Boolean],
                                        forallFun: FunSym[S, Boolean]): Exp[Traversable[T]] /*Join[T, S, TKey, TResult]*/ = {
  ...
  val sizeToEmpty: Exp[_] => Exp[_] = {
    case Sym(Less(Const(0), Sym(Size(coll)))) =>
      coll.nonEmpty
    case Sym(LEq(Const(1), Sym(Size(coll)))) =>
      coll.nonEmpty
    case Sym(Not(Sym(Eq(Sym(Size(coll)), Const(0))))) =>
      coll.nonEmpty   //This case should never trigger - the Eq node will first be transformed by the case below.
    case Sym(Eq(Sym(Size(coll)), Const(0))) =>
      coll.isEmpty
    case e => e
  }
}
